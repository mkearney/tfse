...
#'   this endpoint.
#' @details The GET search/tweets is part of the Twitter REST API 1.1 and
#' is rate limited similarly to other v1.1 methods. See REST API Rate
#' Limiting in v1.1 for information on that model. At this time, users
#' represented by access tokens can make 180 requests/queries per 15
#' minutes. Using application-only auth, an application can make 450
#' queries/requests per 15 minutes on its own behalf without a user context.
...
search_tweets <- function(q, token, geocode = NULL, lang = NULL,
  ...
  params <- paste0("result_type=", result_type, "&count=",
    count, if (!is.null(geocode))
      paste0("&geocode=", geocode), if (!is.null(lang))
      paste0("&lang=", lang), if (!is.null(locale))
      paste0("&locale=", locale), if (!is.null(until))
      paste0("&until=", until), if (!is.null(since_id))
      paste0("&since_id=", since_id), if (!is.null(max_id))
      paste0("&max_id=", max_id), if (include_entities)
      paste0("&include_entities=true") else paste0("&include_entities=false"))
  ...
  while (nrows < count) {
    ...
    if (length(out$search_metadata$next_results) == 0)
      break
...
top_tweet_words <- function(tweets_text, min = 3, exclude_words = NULL) {
  ...
  tweets_text <- unlist(lapply(tweets_text, function(x) unlist(strsplit(x,
    split = " "))))
  tweets_text <- tweets_text[!is.na(tweets_text)]
